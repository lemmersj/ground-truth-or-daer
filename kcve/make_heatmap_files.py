...
def main(args):
    ...
    _, valid_loader = get_data_loaders(dataset=args.dataset,
                                       batch_size=1,
                                       num_workers=1,
                                       return_kp=True)
...
def eval_step(model, data_loader, args):
    ...
    for i, (images, azim_label, elev_label, tilt_label, obj_class,\
        ...
        relative_heatmap_geodesic = upsample(
            relative_heatmap_geodesic, images.shape[2])
        ...
        torch.save(
            relative_heatmap_geodesic, os.path.join(
