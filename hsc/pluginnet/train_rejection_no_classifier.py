...
for Inference under Partial Evidence." Unlike train_rejection.py, this version
is an ablation, which does not use the classifier.
...
def train_step(task_model, rejection_model, data_loader, optimizer,\
    ...
    for _, (input_data, target) in enumerate(data_loader):
        ...
        for index in range(loop_length):
            ...
            loss = bce_loss(rejection_model_output, additional_error)
...
def val_step(task_model, rejection_model, data_loader, device="cuda"):
    ...
    for _, (input_data, target) in enumerate(data_loader):
        ...
        # Loop through all targets in val set
        for i in range(7):
            # Create the fake evidence
            ...
            running_loss += loss_bce.mean().item()
            num_samples += additional_error.shape[0]
...
def main():
    ...
    parser.add_argument('--important_only', type=str, default="False",\
                       help="Only train on examples where there is"\
                        "additional error.")
    parser.add_argument('--comparative_loss', type=str, default="False")
    parser.add_argument('--ae_loss', type=str, default="False")
    ...
    wandb.init(project="gtd-hsc", config=args.__dict__,\
               name="reject-noclassifier-"+str(args.lr)+"-"+str(time.time()))
    ...
    for epoch in range(args.num_epochs + 1):
        ...
        if epoch % args.val_step == 0:
            ...
            with torch.no_grad():
                val_return = val_step(
                    task_model, rejection_model, val_loader,\
                    device=device)
